...
export default function HomeClient() {
  ...
  useEffect(() => {
    const ledModeList = ["2", "8", "venue", "stick"];
    const timeoutId = setTimeout(() => {
      const currentIndex = ledModeList.indexOf(ledMode);
      const nextIndex = (currentIndex + 1) % ledModeList.length;
      setLedMode(ledModeList[nextIndex]);
    }, 5000);
    return () => clearTimeout(timeoutId);
  }, [ledMode]);
      ...
      <div className="relative z-10">
          ...
          >
            ...
            >
              ...
              <div className="pointer-events-none relative h-[240px] w-full shrink-0 xl:absolute xl:bottom-0 xl:right-0 xl:h-full xl:w-full">
                ...
                <Image
                  fill
                  src="/products/Venue.png"
                  alt="Venue"
                  className={`lg:pr-16 object-contain object-center transition-all duration-700 xl:object-right-bottom ${ledMode === "venue" ? "opacity-100" : "opacity-0"}`}
                />
                <Image
                  fill
                  src="/products/LED Stick.png"
                  alt="LED Stick"
                  className={`object-contain object-center transition-all duration-700 xl:object-right-bottom ${ledMode === "stick" ? "opacity-100" : "opacity-0"}`}
                />
              </div>
              ...
              <div className="relative z-10 flex max-w-md flex-col p-6 xl:mt-auto xl:p-0">
                ...
                <div className="mt-6 flex gap-2">
                  ...
                  <button
                    onClick={() => setLedMode("venue")}
                    className={`rounded-full border px-3 py-1 text-[10px] uppercase ${ledMode === "venue" ? "border-brand-cyan text-brand-cyan" : "border-white/10 text-white/30"}`}
                  >
                    LED Venue
                  </button>
                  <button
                    onClick={() => setLedMode("stick")}
                    className={`rounded-full border px-3 py-1 text-[10px] uppercase ${ledMode === "stick" ? "border-brand-cyan text-brand-cyan" : "border-white/10 text-white/30"}`}
                  >
                    LED Stick
                  </button>
                </div>
            ...
            >
              ...
              <div className="relative z-10 flex flex-col p-6 xl:mt-auto xl:p-0">
                ...
                <p className="mt-2 text-sm leading-relaxed text-white/60">
                  Our infrared moving head and transmitter beam out the IR wave
                  to communicate and control wristbands
                </p>
                <div className="mt-6 flex gap-2">
                  <button
        ...
        >
          <motion.div variants={staggerContainer} className="pt-16 text-center">
            ...
            >
              Ready to transform your event? Send us your details and we’ll
              reach out to discuss how we can light up your next project.
            </motion.div>
